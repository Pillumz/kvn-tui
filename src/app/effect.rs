...
pub enum Effect {
    ...
    /// Fetch the Steam rule-sets if absent. Emitted after a successful
    /// connect so the download rides the freshly opened tunnel — pre-tunnel
    /// the source may be unreachable (kill switch or ISP blocks).
    DownloadSteamIfMissing,
